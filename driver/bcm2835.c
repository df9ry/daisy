 ...
 */
uint32_t *bcm2835_peripherals_base = MAP_FAILED;
...
int bcm2835_initialize(void) {
    ...
    */
    ...
    if (fp == NULL) {
    	printk(KERN_ERR "daisy: Unable to open file %s\n",
    			BMC2835_RPI2_DT_FILENAME);
    	return -EBADF;
    }
    if (file_read(fp, BMC2835_RPI2_DT_PERI_BASE_ADDRESS_OFFSET, buf, 4) == -1) {
    	printk(KERN_ERR "daisy: Unable to read bcm2835_peripherals_base\n");
    	return -EBADF;
    }
    ...
    bcm2835_peripherals_size =
    		(buf[0] << 24 | buf[1] << 16 | buf[2] << 8 | buf[3] << 0);
	file_close(fp);
	printk(KERN_DEBUG "daisy: BCM2835 is at %x-%x\n",
			(uint32_t)bcm2835_peripherals_base, bcm2835_peripherals_size);

	//io_memory = request_mem_region(
	//		(uint32_t)bcm2835_peripherals_base, bcm2835_peripherals_size,
	//		"BCM2835");
	//if (!io_memory) {
    //	printk(KERN_ERR "daisy: Unable to map memory\n");
    //	return -EPERM;
	//}
	//printk(KERN_DEBUG "daisy: BCM2835 mapped to %x\n",
